...
python do_create_spdx() {
    ...
    recipe_package = create_spdx_package(
        ...
        license_declared=convert_license_to_spdx(d.getVar("LICENSE")), summary=d.getVar("SUMMARY", True),
        ...
        if subdir == packages_split:
            for package in dirs:
                spdx_package = create_spdx_package(
                    ...
                    version= d.getVar("PV"),
                    id_prefix="Package",
                    license_declared=convert_license_to_spdx(d.getVar("LICENSE:%s" % package) or d.getVar("LICENSE"))
                )
